...
namespace Borealis
{
	...
	public:
		...
		static void DrawQuad(const glm::mat4& transform, const Ref<Texture2D>& texture, const float& tilingFactor = 1.f, const glm::vec4& tint = {1,1,1,1}, int entityID = -1, bool billBoard = false, bool useTextureAR = false);
		...
		static void DrawString(const std::string& string, Ref<Font> font, const glm::mat4& transform, int entityID, float size = 1.f, glm::vec4 colour = {1.f,1.f,1.f,1.f}, bool alignCenter = false, bool outline = true, float outlineWidth = 5.f);
