...
vec3 GetEmission()
{
    return  materials[materialIndex].hasEmissionMap ? texture(emissionMap, GetTexCoord()).rgb * materials[materialIndex].emissionColor.rgb * materials[materialIndex].emissionColor.a: materials[materialIndex].emissionColor.rgb;
}
...
float GetCascadeShadowFactor(vec3 lightDir, vec3 normal)
{
    ...
    float diffuseFactor = dot(normal, -lightDir);
    // Compute derivatives to estimate the slope
    float dx = dFdx(LightPos.z);
    float dy = dFdy(LightPos.z);
    float slope = sqrt(dx * dx + dy * dy);

    // Calculate bias with clamping
    float bias = clamp(slope * 0.005, 0.0006, 0.01);
